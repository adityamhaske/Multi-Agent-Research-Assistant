...
| Page fetching | httpx + BeautifulSoup4 + lxml | current | Async fetch with the SSRF guard from [06_Security.md](../engineering/06_Security.md) |
...
| Testing | pytest, pytest-asyncio, httpx ASGI client | current | See [08](../engineering/08_Testing_and_Quality.md) |
...
| Markdown | react-markdown + remark-gfm | 10.x / 4.x | Safe defaults: **never add `rehype-raw`** (CI-guarded, see [06](../engineering/06_Security.md)) |
...
| Theming | next-themes | current | Class-based dark/light; tokens only — no hardcoded hex in components ([07](../product/07_UIUX_Guidelines.md)) |
| Testing | Vitest + Testing Library; Playwright for E2E | current | See [08](../engineering/08_Testing_and_Quality.md) |
...
| CI | GitHub Actions | lint → typecheck → unit → integration → golden E2E; see [09](../engineering/09_Deployment_and_Operations.md) |
...
- Model IDs and the price table live in config; when a provider deprecates a model,
  ...
  ([08](../engineering/08_Testing_and_Quality.md) §5) run against the new model before merge.

...
def run_config_from_env(*, fake: bool) -> RunConfig:
    ...
    *hosted* paths — `pipeline_runner._run_config_for` and `sidecar._drive_session` —
    ...
    return RunConfig(
        ...
        # 0 = unlimited (docs/04 §6). Defaults match `app/config.py` — the server and local
        # paths are the pair AGENTS.md warns drifts; change both, or the CLI, evals and
        # desktop silently enforce different ceilings than the API.
        max_critic_loops=_int_env("MAX_CRITIC_LOOPS", 2),
        max_cost_per_session_usd=_float_env("MAX_COST_PER_SESSION_USD", 0.0),
        max_wallclock_seconds=_int_env("MAX_WALLCLOCK_SECONDS", 0),
        max_input_tokens=_int_env("MAX_INPUT_TOKENS", 0),

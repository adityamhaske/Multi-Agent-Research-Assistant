"""
The migration ledger (M2C §5, M2E §2).

One row per V1 session **considered** by the migration. Absence from this table means
`NOT_PROCESSED` — never `EMPTY`. That distinction is the reason the table exists.

The ledger is the migration's own audit record: it distinguishes success from emptiness
from unreadability from inconsistency, so a run that could not be read is never counted as
a run that had nothing. M2C.5 measured a 5% missing-checkpoint population against a 6.7%
genuinely-empty one; collapsing them would have reported 93.3% success instead of 88.3%.

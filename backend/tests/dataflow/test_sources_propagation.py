...
The release blocker this guards was a completed report rendering `0 claims,
0 evidence, 0 sources`. Sources are the root of that chain: `run_execution.persist_outcome`
...
and only one of them was covered. `awaiting_approval` is the branch a run takes on its
...
def test_every_outcome_branch_preserves_sources(expected_status, result, extra_state):
    ...
    assert outcome.sources == SOURCES, (
        ...
        "RunOutcome.sources defaults to an empty list this fails silently: nothing persists "

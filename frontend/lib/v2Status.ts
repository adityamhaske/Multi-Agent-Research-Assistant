import type { RunStatusV2 } from "./types";

/**
 * One status vocabulary for a V2 run — the list, the card, the header and the filters
 * (docs/07 §2, Phase 7).
 *
 * The V1 equivalent lives in `lib/status.ts` and is a different union: V2 adds
 ...
 * host can actually produce, and `Record<RunStatusV2, …>` is what makes "added a status
...
export const V2_STATUS_META: Record<RunStatusV2, V2StatusMeta> = {
  PENDING: {
    token: "text-muted",
    label: "Queued",
...
/** Every V2 status, in the order a run moves through them. */
export const V2_STATUS_ORDER: RunStatusV2[] = [
  "PENDING",
  "RUNNING",
  "AWAITING_PLAN",
  "AWAITING_REVIEW",
 ...
 * migration, and a crashed History page is a worse answer than an honest "unrecognised":
 ...
 */
export function v2StatusMeta(status: string): V2StatusMeta {
  return (
    V2_STATUS_META[status as RunStatusV2] ?? {
      token: "text-muted",

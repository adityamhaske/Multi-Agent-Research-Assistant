import type { Source, V2RunGraph } from "./types";

/**
 * The V2 run graph, in the shape the citation renderer already speaks.
 *
 ...
 * the product's central claim made interactive, and it was written against the V1 `Source`
 * shape. The V2 workspace was rendering its report as pre-wrapped plain text instead — no
 * headings, no chips, and, worse, **no ⚠**: a broken citation in a V2 report looked exactly
 * like a working one.
 *
 ...
 * tab says.
 */
export function citedSources(graph: V2RunGraph): Source[] {

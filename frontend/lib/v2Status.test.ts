...
import { V2_STATUS_META, V2_STATUS_ORDER, v2StatusMeta } from "./v2Status";
 ...
 *
 * The V1 equivalent drifted exactly once — `AWAITING_PLAN` was added to the badge and not
 * to the filter list, so the run a user was most likely scanning for could be seen and not
 * filtered for. `Record<RunStatusV2, …>` makes the type checker catch the first half of
 * that; these tests catch the second.
 */

const ALL: RunStatusV2[] = [
  "PENDING",
  "RUNNING",
  "AWAITING_PLAN",
  "AWAITING_REVIEW",
...
describe("V2 status vocabulary", () => {
  it("covers every status the backend can report, in the order given", () => {
    expect([...V2_STATUS_ORDER].sort()).toEqual([...ALL].sort());
  });
  ...
  it("survives a status it has not been taught, without inventing one", () => {
    const meta = v2StatusMeta("SOMETHING_NEW");

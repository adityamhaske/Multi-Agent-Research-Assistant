...
/**
 * Chat over a project's approved research (docs/14 §5).
 *
 * Two differences from the per-report panel, both deliberate:
 *
 * - Citations arrive on the `connected` event, before the first token. They describe
 *   what was *retrieved*, which is settled the moment the query runs — so the chips can
 *   render as the answer streams rather than appearing after it finishes.
 * - A 503 is surfaced as-is instead of being softened. It means no embedding provider is
 *   reachable, and this surface promises answers grounded in approved research. Guessing
 *   without retrieval would break exactly the promise the feature exists to make.
 */
export function ProjectChatPanel({ threadId }: { threadId: string }) {
  ...
  // Switching threads must not leave the previous thread's stream writing into this one.
  useEffect(() => {
    return () => {
  ...
  const send = async () => {
    ...
    setInput("");
    setStreaming({ user: text, assistant: "", citations: [] });
    ...
    if (!res.ok || !res.body) {
      // Rejected before persisting (rate limit, or 503 with no embeddings provider).
      setStreaming(null);
      setInput(text);
    ...
    try {
      await streamSSE(
        ...
        (ev) => {
          ...
          if (data.type === "connected" && Array.isArray(data.citations)) {
    ...
    // Both messages are persisted server-side; refetch before clearing local streaming
    // state so the transcript never flickers empty.
    await qc.invalidateQueries({ queryKey: queryKeys.threadMessages(threadId) });
      ...
      >
        ...
        {messages.map((m) =>
          m.role === "user" ? (
            ...
            <AssistantBubble key={m.id} text={m.content} citations={m.citations ?? []} />
          ),
        )}
        {streaming && (
          <>
            <UserBubble text={streaming.user} />
            <AssistantBubble text={streaming.assistant} citations={streaming.citations} />
          </>
        )}
      </div>

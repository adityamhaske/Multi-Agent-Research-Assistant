...
vi.mock("@/hooks/runs", () => {
  return {
    useReportReview: () => ({ mutate, isPending: false, isError: false, error: null }),
    usePlanReview: () => ({ mutate: planMutate, isPending: false, isError: false, error: null }),
...
describe("RunWorkspace", () => {
  ...
  it("offers all three exports once an artifact exists", () => {
    ...
    expect(screen.getByRole("link", { name: "Markdown" })).toHaveAttribute(
      "href",
      expect.stringContaining("/runs/run-1/export.md"),
    );
    ...
    expect(screen.getByRole("link", { name: "Verification bundle" })).toHaveAttribute(
      "href",
      expect.stringContaining("/runs/run-1/bundle.json"),
    );
  });

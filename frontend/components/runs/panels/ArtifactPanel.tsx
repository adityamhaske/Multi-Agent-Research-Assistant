...
import { useV2Verification } from "@/hooks/runs";
...
export function ArtifactPanel({ graph }: { graph: RunGraph }) {
  ...
  const download = async (format: Format, event: React.MouseEvent) => {
    ...
    try {
      await downloadExport(
        `/runs/${graph.run.id}/${format === "bundle.json" ? "bundle.json" : `export.${format}`}`,
      ...
      <section className="card" aria-labelledby="download-heading">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <a
            ...
            href={`${base}/runs/${graph.run.id}/bundle.json`}
          ...
          <a
            ...
            href={`${base}/runs/${graph.run.id}/export.md`}
          ...
          <a
            className="btn btn-secondary"
            href={`${base}/runs/${graph.run.id}/export.pdf`}

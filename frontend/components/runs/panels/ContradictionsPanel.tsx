...
import type { RunContradiction, RunGraph, RunSource } from "@/lib/types";
...
function Side({
  ...
  label: string;
  source: RunSource | null | undefined;
  quote: string | null;
  summary: string | null;
}) {
  return (
...
export function ContradictionsPanel({ graph }: { graph: RunGraph }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <ul className="space-y-3">
        {detected.map((c: RunContradiction, i) => {

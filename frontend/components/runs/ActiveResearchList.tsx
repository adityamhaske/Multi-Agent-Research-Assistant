...
export function ActiveResearchList({
  ...
  const oldestFirst = (a: RunSummary, b: RunSummary) =>
    new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
  const newestFirst = (a: RunSummary, b: RunSummary) =>
    new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
  const isWaiting = (r: RunSummary) => runStatusMeta(r.status).needsYou;
  const isInProgress = (r: RunSummary) => r.status === "PENDING" || r.status === "RUNNING";
  const isFinished = (r: RunSummary) =>
  ...
  // but the wire is not the type, and `runStatusMeta` already documents why a status this
  // build has not been taught is the ordinary shape of a rolling deploy, not a bug. A run in

...
import type { MemoryStatus, V2RunSummary } from "@/lib/types";
...
function run(cost: number, id: string): V2RunSummary {
  return {
    id,
    project_id: "p1",
    question: "Q",
...
describe("ProjectHealth", () => {
  ...
  it("names the exact set the spend covers, including that legacy sessions are outside it", () => {
    view([run(1, "a"), run(1, "b")]);
    // The old caption said "runs shown on this page", which was wrong in both directions:
    // the list below shows fewer, and legacy sessions are never in this sum at all.
    expect(screen.getByText(/2 most recent research runs/)).toBeInTheDocument();
    expect(screen.getByText(/not counting legacy sessions/)).toBeInTheDocument();
  });
  ...
  it("keeps corpus emptiness and memory emptiness in separate words", () => {
    ...
    expect(screen.getByText(/Built from approved legacy sessions/)).toBeInTheDocument();
  });

  it("scopes the memory counters to legacy sessions, so they cannot contradict an Approved run badge", () => {
    // `approved_reports` counts COMPLETED V1 sessions only, and nothing in the V2 runtime
    // writes memory — so an unscoped "nothing approved yet" sits beside run rows badged
    // "Approved" and one of the two reads as a lie.
    memoryState = {
      data: memory({ approved_reports: 0, indexed_reports: 0 }),
      isLoading: false,
      isError: false,
    ...
    view();
    expect(
      screen.getByText(/research runs are not indexed into it/),
    ).toBeInTheDocument();
  });
  ...
  it("blames the missing embedding model, not the user, when memory cannot function at all", () => {
    ...
    expect(screen.queryByText(/Built from approved legacy sessions/)).not.toBeInTheDocument();
  });

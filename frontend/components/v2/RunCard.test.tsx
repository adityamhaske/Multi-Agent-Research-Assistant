...
vi.mock("@/hooks/v2", async () => {
  const actual = await vi.importActual<typeof import("@/hooks/v2")>("@/hooks/v2");
  return {
    ...actual,
    useArchiveV2Run: () => ({
...
function createSummary(overrides: Partial<V2RunSummary> = {}): V2RunSummary {
  return {
    id: "run-123",
    project_id: "proj-456",
...
function renderCard(run: V2RunSummary, showProject?: string | null) {

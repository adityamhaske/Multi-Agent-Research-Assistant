...
export function LegacySessions() {
  ...
  // Nothing to show and nothing archived to reveal: a legacy panel on an account
  // that never used V1 is pure noise, so it renders nothing at all.
  if (!isLoading && !isError && rows.length === 0 && !showArchived) return null;

  return (
    <section aria-labelledby="legacy-sessions" className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 max-w-2xl text-xs leading-relaxed text-text-muted">
            Runs from the earlier research pipeline{active && <> in {active.name}</>}. They are
            kept readable and exportable; new research is recorded above.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="card text-center">
          {showArchived ? (
            ...
            <>
              <p className="text-sm text-text-secondary">
                No legacy sessions in {active ? `“${active.name}”` : "this project"}.
              </p>
            </>
          )}
        </div>

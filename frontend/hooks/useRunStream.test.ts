...
describe("useRunStream", () => {
  it("keeps reading past a design gate the run has already left", () => {
    const { result } = renderHook(() => useRunStream("run-1", true, "FAILED"), { wrapper });
  ...
  it("still closes at a gate the run is actually parked at", () => {
    ...
    const { result } = renderHook(() => useRunStream("run-1", true, "AWAITING_PLAN"), {
      wrapper,
    });
  ...
  it("closes at the report gate only when parked there", () => {
    const parked = renderHook(() => useRunStream("run-a", true, "AWAITING_REVIEW"), { wrapper });
    opened[0].send(1, { type: "HITL_READY" });
    expect(opened[0].closed).toBe(true);
    expect(parked.result.current.events).toHaveLength(1);

    const movedOn = renderHook(() => useRunStream("run-b", true, "COMPLETED"), { wrapper });
  ...
  it("closes on a true terminal, whatever the status says", () => {
    ...
    renderHook(() => useRunStream("run-1", true, "RUNNING"), { wrapper });
